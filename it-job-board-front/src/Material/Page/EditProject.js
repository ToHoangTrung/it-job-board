...
const EditProject = ({match}) => {
    ...
    useEffect(() => {
        const fetchProject = async () => {
            try{
                ...
                fetchProject.users.map(item => item.displayName = item.visa.concat(": ").concat(item.firstName).concat(" ").concat(item.lastName));
                setProjectInfo(fetchProject);
                setIsLoading(false);
            }catch (err){
    ...
    return (
        <div className={"new-edit-project"}>
            <p className={"headline"}>{t('newEditProjectScreen.editProject')}</p>
                    ...
                    <>
                        ...
                        <div className={"form-event-btn"}>
                            <Button variant="contained" onClick={() => history.push("/projects")}>
                                {t('newEditProjectScreen.button.cancel')}
                            </Button>
                            <Button variant="contained" type={"submit"} form={"form-edit"}>
                                {t('newEditProjectScreen.button.acceptEdit')}
                            </Button>
                        </div>
                    </>
                ) : (
