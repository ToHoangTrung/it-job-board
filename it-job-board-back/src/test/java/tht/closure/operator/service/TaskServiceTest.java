...
import tht.closure.operator.repository.ProjectRepository;
import tht.closure.operator.ApplicationWebConfig;
import tht.closure.operator.model.entity.QTask;
import tht.closure.operator.model.entity.QTaskAudit;
import tht.closure.operator.model.exception.DeadlineAfterFinishingDateException;
import tht.closure.operator.repository.TaskRepository;
...
public class TaskServiceTest {
	...
	public void testUpdateDeadline() {
		...
		catch (DeadlineAfterFinishingDateException | ResourceNotFoundException e) {
			em.clear();
			System.err.println(e.getMessage());
			Task task1 = taskRepository.findById(taskId).orElse(null);
			Assert.assertNotNull("This task does not found: ", task1);
